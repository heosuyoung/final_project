  ...
  data() {
    return {
      ...
      commoditiesData: {
        ...
        crude_oil: { price: '0.00', change: 0 },
        natural_gas: { price: '0.00', change: 0 }
      },
      indexData: {
  ...
  methods: {
    ...
    async fetchCommoditiesData() {
      try {
        ...
        if (response.data.success) {
          // 실시간 데이터로 상품 가격 정보 업데이트
          Object.keys(response.data.data).forEach(commodity => {
            if (this.commoditiesData[commodity]) {
              const data = response.data.data[commodity];
              this.commoditiesData[commodity] = {
          ...
          console.log('실시간 상품 가격 데이터 업데이트 완료:', response.data.data);
        }
      } catch (error) {
        console.error('상품 가격 데이터 가져오기 실패:', error);
        // 기본값으로 복원
        this.commoditiesData = {
          gold: { price: '2,345.60', change: 0.75 },
          silver: { price: '28.40', change: -0.32 },
          copper: { price: '4.12', change: 1.25 },
          crude_oil: { price: '76.82', change: 1.65 },
          natural_gas: { price: '2.48', change: -1.98 }
        };
      }
    },
    ...
    updateCommodityData() {
      ...
      const commodityData = {
        ...
        'crude_oil': {
          ...
          backgroundColor: 'rgba(54, 162, 235, 0.2)',
          borderColor: 'rgb(54, 162, 235)',
          data: [73.2, 74.8, 76.1, 77.5, 76.8, 75.9]
        },
        'natural_gas': {
          label: '천연가스(USD)',
          backgroundColor: 'rgba(75, 192, 192, 0.2)',
          borderColor: 'rgb(75, 192, 192)',
          data: [2.65, 2.58, 2.52, 2.48, 2.53, 2.47]
        }
      };
